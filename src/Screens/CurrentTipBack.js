...
const CurrentTipBack = (props) => {
  ...
  const navigateTo = () => history.push('/CurrentTip1');//eg.history.push('/login');
  const SubmitUpdateStartLoading = async () => {
    // const params = {
    //   Start_Load_Time: Date.now(),
    // };
    const params = {
      "Start_Load-time": momentTimezone().tz("America/New_York").format(),
      "Index":"A2"
    };
    setLoading(true);
