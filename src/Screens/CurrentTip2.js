...
const CurrentTip2 = (props) => {
  ...
  let startUnloadingApiUrl = URL("Trips_Ben/6155eed0434cc01d843d1fe0/start_unload");
  ...
  const navigateTo = () => history.push('/CurrentTip3');//eg.history.push('/login');
  const SubmitUpdateStartUnloading = async () => {
    // const params = {
    //   Start_Load_Time: Date.now(),
    // };
    const params = {
      "Start_Unload-time": momentTimezone().tz("America/New_York").format(),
    };
    setLoading(true);
    const responseData = await Patch(startUnloadingApiUrl, params, apiHeader);
                  ...
                  <div className="col-md-12">
                    <div className="currentTripMiddle">
                      <div>
                        ...
                        >
                          <h1>Press “Start Unloading” when you are ready</h1>
                          <div className="col-md-12 statusButtonContainer">
                            {currentStatus == 0 && (
                              <Button
                                onClick={() => {
                                  SubmitUpdateStartUnloading();
                                }}
                                disabled={submitLoading}
                              >
                                <img src={loadingBox} alt="" />
                                <span onClick={handleShow}> Start Unloading</span>
                              </Button>
                            )}
                            {/* {currentStatus == 0 && (
                              <Button
